import { useState } from 'react';
import { ShieldCheck, User, Search, Users, Building2, Lock, ArrowRight, ExternalLink } from 'lucide-react';
import { Button } from '@/components/ui/button';

interface EcosystemOnboardingProps {
  isLifeAppVerified?: boolean;
  hasBusinessTag?: boolean;
}

interface Role {
  id: string;
  name: string;
  price?: string;
  icon: React.ReactNode;
  description: string;
  available: boolean;
  requiresBusiness?: boolean;
}

const EcosystemOnboarding = ({ isLifeAppVerified = false, hasBusinessTag = false }: EcosystemOnboardingProps) => {
  const [selectedRole, setSelectedRole] = useState<string | null>(null);

  const roles: Role[] = [
    {
      id: 'individual',
      name: 'Standard Individual',
      icon: <User className="w-5 h-5 text-muted-foreground" />,
      description: 'Access the Hub for $10 a la carte queries.',
      available: true,
    },
    {
      id: 'analyst',
      name: 'Analyst Enrollment',
      price: '$9,995/yr',
      icon: <Search className="w-5 h-5 text-primary" />,
      description: 'Unlock 5,000 CRD and Foundational Filters.',
      available: true,
    },
    {
      id: 'professional',
      name: 'Professional Enrollment',
      price: '$24,995/yr',
      icon: <Users className="w-5 h-5 text-emerald-400" />,
      description: 'Unlock 20,000 CRD and Merchant Integrations.',
      available: true,
    },
    {
      id: 'enterprise',
      name: 'Enterprise Client',
      price: '$49,995+/yr',
      icon: <Building2 className="w-5 h-5 text-amber-400" />,
      description: 'Unlock 50,000 CRD and T-1-P Verification.',
      available: hasBusinessTag,
      requiresBusiness: true,
    },
  ];

  if (!isLifeAppVerified) {
    return (
      <div className="max-w-md mx-auto mt-20 p-8 bg-card border border-border rounded-2xl text-center">
        <ShieldCheck className="w-12 h-12 text-primary mx-auto mb-4" />
        <h2 className="text-xl font-bold text-foreground mb-2">IDIA Life App Required</h2>
        <p className="text-muted-foreground text-sm mb-6">
          All ecosystem users must verify their biological identity via IDIA Life before accessing the Hub.
        </p>
        <Button className="w-full gap-2">
          Download IDIA Life <ExternalLink className="w-4 h-4" />
        </Button>
      </div>
    );
  }

  return (
    <div className="max-w-5xl mx-auto p-6">
      <div className="mb-10 text-center">
        <h1 className="text-3xl font-bold text-foreground">Configure Your Hub Access</h1>
        <p className="text-muted-foreground mt-2">Elevate your individual account with professional data capabilities.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {roles.map((role) => (
          <div
            key={role.id}
            onClick={() => role.available && setSelectedRole(role.id)}
            className={`relative p-5 rounded-xl border-2 transition-all flex flex-col justify-between h-64 ${
              !role.available
                ? 'opacity-50 cursor-not-allowed bg-background border-border'
                : selectedRole === role.id
                  ? 'border-primary bg-primary/5 cursor-pointer'
                  : 'border-border bg-card hover:border-muted-foreground/30 cursor-pointer'
            }`}
          >
            <div>
              <div className="flex justify-between items-start mb-4">
                <div className={`p-2 rounded-lg ${!role.available ? 'bg-background' : 'bg-muted'}`}>
                  {role.icon}
                </div>
                {!role.available && <Lock className="w-4 h-4 text-muted-foreground/50" />}
              </div>
              <h3 className="font-bold text-foreground">{role.name}</h3>
              {role.price && <div className="text-xs font-mono text-emerald-400 mt-1">{role.price}</div>}
              <p className="text-xs text-muted-foreground mt-3 leading-relaxed">{role.description}</p>
            </div>

            {!role.available && role.requiresBusiness && (
              <div className="mt-auto">
                <button className="text-[10px] text-primary hover:underline">Apply for Business Tag</button>
              </div>
            )}
          </div>
        ))}
      </div>

      <div className="mt-12 flex justify-center">
        <Button
          disabled={!selectedRole}
          size="lg"
          className="px-10 gap-2"
        >
          {selectedRole === 'enterprise' ? 'Begin T-1-P Enrollment' : 'Confirm & Continue'}
          <ArrowRight className="w-4 h-4" />
        </Button>
      </div>
    </div>
  );
};

export default EcosystemOnboarding;