...
const IdentityStatusPills = ({ bioKeyStatus = "STABLE", kycTier = 1 }: IdentityStatusPillsProps) => {
  ...
  return (
    <div className="flex items-center gap-2 md:gap-3">
      {/* Bio-Key Indicator */}
      <div
        className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full border border-border ${currentBio.bg} transition-all`}
      >
        <Fingerprint className={`w-3.5 h-3.5 ${currentBio.color}`} />
        <span className={`text-[10px] font-bold uppercase tracking-wider ${currentBio.color}`}>{currentBio.label}</span>
        {bioKeyStatus === "STABLE" && <CircleDot className="w-2 h-2 text-emerald-500 animate-pulse" />}
      </div>
      ...
      <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-full border border-border bg-muted/50">
        ...
        <span className="text-[10px] font-bold uppercase tracking-wider text-foreground/70">KYC Tier {kycTier}</span>
        {kycTier === 2 && (
      ...
      <div className="hidden xl:block group relative">
        ...
        <div className="absolute right-0 top-8 w-48 p-3 bg-popover border border-border rounded-lg shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            Detailed Bio-metric and Identity metrics are managed exclusively within the <strong>Life by IDIA</strong>{" "}
            mobile app.
          </p>
        </div>
      </div>
