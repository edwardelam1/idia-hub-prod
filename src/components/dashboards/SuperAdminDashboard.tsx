  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent><SynapseVisualizer /></CardContent>
      </Card>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="health-data" className="space-y-6"><HealthDataDashboard /></TabsContent>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card><CardHeader><CardTitle>Pending Actions</CardTitle><CardDescription>Items requiring immediate attention</CardDescription></CardHeader><CardContent className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg"><div className="flex items-center space-x-3"><AlertTriangle className="h-5 w-5 text-yellow-600" /><div><p className="font-medium">Account Conversion Requests</p><p className="text-sm text-gray-600">{overviewStats.pendingRequests} pending approval</p></div></div><Button size="sm" onClick={() => setActiveTab('organizations')}>Review</Button></div>
              <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg"><div className="flex items-center space-x-3"><Brain className="h-5 w-5 text-purple-600" /><div><p className="font-medium">AI Generated Bundles</p><p className="text-sm text-gray-600">{overviewStats.aiGeneratedBundles} awaiting review</p></div></div><Button size="sm" onClick={() => setActiveTab('ai-management')}>Review</Button></div>
            </CardContent></Card>
            <Card><CardHeader><CardTitle>Recent Activity</CardTitle><CardDescription>Latest platform activities</CardDescription></CardHeader><CardContent><div className="text-center py-4 text-gray-500">{recentActivity.length === 0 ? <p>Activity feed awaiting live platform events</p> : <div className="space-y-4">{recentActivity.map((activity) => (<div key={activity.id} className="flex items-start space-x-3">{getActivityIcon(activity.type)}<div className="flex-1 min-w-0"><p className="text-sm font-medium">{activity.action}</p><p className="text-xs text-gray-600">{activity.details}</p><p className="text-xs text-gray-500 mt-1">{activity.timestamp}</p></div></div>))}</div>}</div></CardContent></Card>
          </div>

          <Card><CardHeader><CardTitle>System Status Overview</CardTitle><CardDescription>System services awaiting live monitoring data</CardDescription></CardHeader><CardContent><div className="text-center py-8 text-gray-500"><p>System monitoring awaiting live pipeline data</p></div></CardContent></Card>
        </TabsContent>

        <TabsContent value="audit-logs" className="space-y-6"><AuditLogs /></TabsContent>
      </Tabs>
    </div>
  );
};
