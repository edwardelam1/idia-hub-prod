...
async function fetchOmniRecords(
  ...
  console.info(
    `[BEGIN: OmniFetch.Execution] Initiating cross-table data retrieval for operator IDs: [${idsToCheck.join(", ")}]`,
  );
  try {
    const filterConditions = idsToCheck
      .flatMap((id) => [`user_id.eq.${id}`, `entity_id.eq.${id}`, `pseudo_user_id.eq.${id}`])
      .join(",");
    ...
    if (healthRes.error)
      console.error(
        `[ERROR: OmniFetch.Health.Query] Health table query failed: ${healthRes.error.message} | Code: ${healthRes.error.code}`,
      );
    if (lifestyleRes.error)
      console.error(
        `[ERROR: OmniFetch.Lifestyle.Query] Lifestyle table query failed: ${lifestyleRes.error.message} | Code: ${lifestyleRes.error.code}`,
      );

    console.info(
      `[END: OmniFetch.Execution] Retrieval complete. Health payload: ${healthRes.data?.length || 0}, Lifestyle payload: ${lifestyleRes.data?.length || 0}`,
    );
    ...
    console.error(
      `[CRITICAL FAILURE: OmniFetch.Execution] Fatal exception during parallel retrieval stack: ${err instanceof Error ? err.stack : String(err)}`,
    );
...
serve(async (req) => {
  ...
  try {
    ...
    const activePlatformGuid = context?.platformGuid;
    const validIds = [activeUserId, activePlatformGuid].filter(Boolean) as string[];

    if (validIds.length > 0 && SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
      console.info(`[BEGIN: BestFriendAI.OmniFetchRouting] Invoking OmniFetch fallback for valid IDs.`);
      const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
      const audit = await fetchOmniRecords(supabase, validIds);

      if (audit.success) {
        if (audit.health.length > 0) sourceHealth = audit.health;
        if (audit.lifestyle.length > 0) sourceLifestyle = audit.lifestyle;
        console.info(
          `[STATUS: BestFriendAI.OmniFetchRouting] DB override successful. Active context set to ${audit.health.length} health + ${audit.lifestyle.length} lifestyle records.`,
        );
      } else {
        console.warn(
          `[WARNING: BestFriendAI.OmniFetchRouting] Omni-fetch failed or returned empty: ${audit.error ?? "No data matched IDs"}`,
        );
      }
      console.info("[END: BestFriendAI.OmniFetchRouting] OmniFetch phase complete.");
    }

    console.info("[BEGIN: BestFriendAI.DataAggregation] Summarizing full dataset before truncation.");
    const marketplaceSummary = isDataScientistMode ? summarizeMarketplaceData(sourceHealth, sourceLifestyle) : null;
    console.info(
      `[END: BestFriendAI.DataAggregation] Total records aggregated into summary: Health=${sourceHealth.length}, Lifestyle=${sourceLifestyle.length}`,
    );
    ...
    console.info(
      `[END: BestFriendAI.DataTruncation] Final JSON dimensions passed to prompt: ${healthMetrics.length} health, ${lifestyleEvents.length} lifestyle.`,
    );
    ...
    const messages = [
      ...
      ...formattedHistory,
      ...(shouldAppendCurrentMessage ? [{ role: "user", content: contextString }] : []),
    ];
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "gpt-4o-mini", messages, temperature: 0.3, max_tokens: 4096 }),
    });
