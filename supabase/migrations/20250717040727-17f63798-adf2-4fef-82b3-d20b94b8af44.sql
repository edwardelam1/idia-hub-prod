...
-- Reset recent records to trigger comprehensive processing
...
-- Clear recent queue items to rebuild with comprehensive processing
DELETE FROM data_processing_queue 
WHERE created_at > NOW() - INTERVAL '48 hours';

-- Insert recent health data into processing queue for comprehensive extraction
...
-- Trigger edge function to process comprehensive health data
...
) as comprehensive_processing_triggered;

-- Trigger comprehensive bundle generation
SELECT net.http_post(
  'https://zxyngqciipcvveigrzqt.supabase.co/functions/v1/trigger-comprehensive-bundle-generation',
  '{"trigger": "live_healthkit_data"}'::jsonb,
  '5000'::integer,
  '{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inp4eW5ncWNpaXBjdnZlaWdyenF0Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MTMyMjA3NiwiZXhwIjoyMDY2ODk4MDc2fQ.G0Nj4rG9N6N2MKlf9xYyPRXA9WJQx8ZhxqCMM-tpMWY"}'::jsonb
) as bundle_generation_triggered;