...
-- Add a unique constraint to prevent future duplicates in health_metrics
-- Using a partial unique index to handle nulls properly
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_health_metrics_unique_entry 
ON health_metrics (user_id, step_count, DATE_TRUNC('minute', recorded_at))
WHERE step_count IS NOT NULL AND recorded_at IS NOT NULL;
