...
serve(async (req) => {
  ...
  try {
    ...
    masterNonce++; // Increment local counter
    ...
    for (const contributor of contributing_users) {
      ...
      if (!lifeWallet || !/^0x[a-fA-F0-9]{40}$/.test(lifeWallet)) {
        console.warn(`[SKIP] User ${contributor.user_id} has no valid wallet. Defaulting to escrow logic.`);
        // Note: You may want to redirect skipped yields to a DAO/Escrow wallet here
        continue;
      }
