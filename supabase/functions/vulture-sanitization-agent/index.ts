...
function reconstructTimestamp(row: Record<string, any>, index: number, fileCreated: string): string {
  const candidates = ['timestamp', 'created_at', 'date', 'datetime', 'event_time']
...
function mapToBusinessRow(row: Record<string, any>, pseudoId: string): Record<string, any> {
  return {
    pseudo_business_id: pseudoId,
    business_category: String(row.category ?? row.business_category ?? 'rehabilitated'),
    transaction_patterns: row,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (source !== 'pg_trigger' && !req.headers.get('authorization')) {
      ...
      return new Response(JSON.stringify({ error: 'unauthorized' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    const { data: opened, error: openErr } = await supabase
      ...
      .insert({ original_file_name: fileName, bucket_path: `${bucket}/${fileName}`, status: 'processing', action: 'sanitize' })
      .select('id').single()
    if (openErr) { console.error('[BEGIN: Vulture.LedgerOpen.Stall]', openErr); throw openErr }
    ledgerId = opened.id
    ...
    if (saltErr) { console.error('[BEGIN: Vulture.VaultAccess.Stall]', saltErr); throw saltErr }
    ...
    if (dlErr || !blob) { console.error('[BEGIN: Vulture.StorageDownload.Stall]', dlErr); throw dlErr ?? new Error('download failed') }
    ...
    const batchPseudo = await sha256Hex(`vulture:${fileName}:${originalHash}`)
    const businessRows = tagged.map((r) => mapToBusinessRow(r, batchPseudo.slice(0, 32)))
    const BATCH = 500
    for (let i = 0; i < businessRows.length; i += BATCH) {
      const chunk = businessRows.slice(i, i + BATCH)
      const { error: insErr } = await supabase.from('staged_business_data').insert(chunk)
      if (insErr) { console.error('[BEGIN: Vulture.BatchInsert.Stall]', { offset: i, insErr }); throw insErr }
    ...
    const manifest = {
      claim_generator: 'IDIA-Vulture/1.0',
      ...
      assertions: [
        ...
        { label: 'idia.liability_shield', data: { status: 'ACQUIRED_REHABILITATED', pii_stripped: true, temporal_reconstructed: true } },
      ],
    }
    const manifestPath = `${fileName}.manifest.json`
    const { error: mErr } = await supabase.storage.from(MANIFEST_BUCKET).upload(
      manifestPath, new Blob([JSON.stringify(manifest, null, 2)], { type: 'application/json' }),
      { upsert: true }
    )
    if (mErr) { console.error('[BEGIN: Vulture.ManifestUpload.Stall]', mErr); throw mErr }
