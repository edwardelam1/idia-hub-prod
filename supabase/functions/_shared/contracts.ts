/**
 * IDIA Governance Protocol — Contract Configuration (Deno edge mirror)
 *
 * KEEP IN SYNC WITH: src/config/contracts.ts
 *
 * Edge functions cannot import from src/, so this file mirrors the
 * frontend config. Pure data + ABI exports — no runtime deps.
 *
 * Hub edge functions are hard-mainnet under the existing chain-ID 8453
 * guard in idia-circular-settlement. The testnet block is mirrored only
 * so the typed shape matches the frontend one-to-one.
 */
...
if ((ACTIVE_DEPLOYMENT as DeploymentEnv) !== "mainnet") {
  console.error("[PROTOCOL][BOOT][END:FAIL] Non-mainnet deployment — ACTIVE_DEPLOYMENT =", ACTIVE_DEPLOYMENT);
} else {
