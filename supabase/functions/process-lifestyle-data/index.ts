  ...
  try {
    ...
    for (const item of pendingItems || []) {
      try {
        ...
        const { error: insertError } = await supabaseClient.from("staged_lifestyle_data").insert([processedData]);
...
async function processLifestyleEvent(deviceEvent: any) {
  ...
  const locationZone = payload.location
    ? anonymizeLocation(payload.location.latitude, payload.location.longitude)
    : null;
